...
class Test(unittest.TestCase):
    ...
    def analyseEpuMovieFile(self, movie_name):
        ...
        if m is not None:
            ...
            movie_id = "{prefix}_{id1}_Data_{id2}_{id3}_{date}_{hour}".format(**dictResult)
        return movie_id
    ...
    def analyseEPUData(self, image_disc_path):
                ...
                for data_entry in gridsquare_path.iterdir():
                    if data_entry.name == "Data":
                        ...
                        list_movie_dict = sorted(
                            list_movie_dict,
                            key=lambda movie_dict: "_".join(movie_dict['movie_id'].split("_")[-2:])
                        )
        ...
        dict_data["gridsquares"] = sorted(
            ...
            key=lambda gridsquare: "_".join(gridsquare["movies"][0]['movie_id'].split("_")[-2:])
        )
        return dict_data
        ...
        for gridSquare in dictData["gridsquares"]:
            for path in gridSquare["files"]:
                ...
                if imageName.endswith(".jpg"):
                    allParams[gridSquare["name"]] = {
                        "listGalleryPath": [str(path)]
                    }
                    break
